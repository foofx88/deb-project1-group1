            ...
            for i in range(len(response['Contents'])):
                if aws_log_file == response['Contents'][i]['Key']:
                    try:
                        ...
                        df_updated_incremental_log.to_csv(aws_log_file, index=False)
                        write_into_s3 = s3.upload_file(aws_log_file, Bucket=aws_bucket, Key=aws_log_file)
                    ...
                    try:
                        ...
                        logging.error(f"There has been an error creating the Incremental Log. Status - {status}")
